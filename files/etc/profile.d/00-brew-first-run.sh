...
if test "$(id -u)" -gt "0"; then
  ...
  if test ! -d /home/linuxbrew/.linuxbrew; then
    name="$(hostname -s)"
    linuxbrew_home="${XDG_DATA_HOME:-$HOME/.local/share}"/bingobox/"${name}"
    printf "Setting up Linuxbrew...\t\t\t\t "
    if test ! -d "${linuxbrew_home}"; then
      mkdir -p "${linuxbrew_home}"
      if test -d "${XDG_DATA_HOME:-$HOME/.local/share}"/bingobox/.linuxbrew; then
        mv "${XDG_DATA_HOME:-$HOME/.local/share}"/bingobox/.linuxbrew "${linuxbrew_home}"/.linuxbrew
      fi
    fi
